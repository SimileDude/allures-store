...
const Footer = () => (
    <footer className="bg-dark text-light flex flex-col md:flex-row justify-between p-8">
        <BrandLink classes="text-earthYellow mb-4 md:mb-0 md:pl-4 inline-block order-1" />
        ...
        <div className="flex flex-col md:flex-row justify-between md:mx-auto space-x-0 md:space-x-8 space-y-8 md:space-y-0 order-3 md:order-2">
            {links.map((item) => (
                <div className="flex flex-col flex-wrap" key={item.title}>
                    <h6 className="font-alegreya text-2xl mb-2">{item.title}</h6>
                    <div>
                        {item.links.map((link) => (
                            <Link
                                to={link.path}
                                key={link.name}
                                className="lowercase pb-1 text-sm block hover:text-earthYellow"
                            >
                                {link.name}
                            </Link>
                        ))}
        ...
        <div className="text-earthYellow space-x-4 md:space-x-0 order-2 md:order-3 flex md:block mb-6 md:mb-0">
            <a
